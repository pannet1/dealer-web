...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (txn.checked == true) sync_frm_clr("BUY");
});
...
copyButton.addEventListener("click", () => {
  ...
  const rowsToCopy = Array.from(sourceTable.querySelectorAll("tr")).filter(
    (row) => {
  ...
  rowsToCopy.forEach((row, index) => {
    ...
    let otype;
    const otypeInputs = Array.from(
      document.querySelectorAll('input[name="otype"]'),
    );
    ...
    statDescDiv.textContent =
      clientNameInput.value + " / " + otype + " / " + ptype;

    if (txnInputField.value == "SELL") {
      statTitleDiv.className = "stat-title text-red-500";
      newRow.appendChild(statTitleDiv);

      statValueDiv.textContent =
        "-" + qtyInput.value + " " + symbolInputField.value;
